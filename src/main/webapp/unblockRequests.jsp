...
<header class="header">
      ...
      <ul class="main-nav">
          ...
          <li><a id="active" href="Logics?unblockRequests=true">Unblock Requests</a></li>
...
<div class="drives">
	...
	if(request.getAttribute("requests")==null){%>
		<div class="head"><center>There is no requests</center></div>
	<%}else{ %>
		<div class="heading"><center>Unblock Requests</center></div>
	<%ArrayList requests;
	requests=(ArrayList)request.getAttribute("requests");
	for(Object obj : requests){
		String[] reques=(String[])obj;%>
		<a href="Logics?usn=<%= reques[0]%>&name=<%=reques[1] %>&unBlockRequestDetails=true"><table>
			<tr><td><%= reques[0]%></td><td><%= reques[1] %></td></tr>
		</table></a>
	<%}
	}%>
</div>
