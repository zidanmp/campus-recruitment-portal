...
<script type="text/javascript">
	function checkInput(){
		...
		if(isNaN(cgpa))
		{
			alert("Please Provide a proper cgpa");
			return false;
		}
	}
</script>
...
<body>
	<header class="header">
      ...
      <ul class="main-nav">
          ...
          <li><a id="active" href="Logics?usn=<%=session.getAttribute("username") %>&studentProfile=true">profile</a></li>
	...
	<h3><%if(request.getAttribute("student")==null){ out.println("ERROR!");}else{  %></h3>	
  <h2>Student Profile</h2>
  <form method="post" name="checkingInput" action="Logics" onsubmit="return checkInput()">
  	<div class="user-box">
      <input style=color:green; type="text" name="usn" value=<%=student.getUsn()%> readonly>
      <label>usn</label>
    </div>
    <%if(student.getAttendance()==3){ %>
    <div class="user-box">
      <input type="text" style=color:red; value=BLOCKED readonly>
    ...
    <div class="user-box">
		<select name="dept">
		<%if(student.getDept_name().equals("cse")){%>
		<option value=cse selected>CSE</option><%}else{ %>
		<option value=cse>CSE</option>
		<%}if(student.getDept_name().equals("ise")){ %>
		<option value=ise selected>ISE</option><%}else{ %>
		<option value=ise>ISE</option>
		<%}if(student.getDept_name().equals("ece")){ %>
		<option value=ece selected>ECE</option><%}else{ %>
		<option value=ece>ECE</option>
		<%}if(student.getDept_name().equals("eee")){ %>
		<option value=eee selected>EEE</option><%}else{ %>
		<option value=eee>EEE</option>
		<%}if(student.getDept_name().equals("me")){ %>
		<option value=me selected>ME</option><%}else{ %>
    ...
    <div class="user-box">
      <input type="text" name="attendance" style=color:#BE5504; value="<%= student.getAttendance()%>" readonly>
      <label>ATTENDANCE</label>
    </div>
    <%if(student.getAttendance()==3){ %>
    <input id=forgo type="submit" name=raiseUnblockRequest value="Request to unblock">
    <input id=forgo type="submit" name=updateProfile value="update">
    <%}else {%>
    <input id=forgot type="submit" name=updateProfile value="update"><%} %>
  </form>
  <%}}} %>
</div>
	</div>
